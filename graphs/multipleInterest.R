...
InterestCount= c("one","four")



for (ped in pedCount){
    f = file(paste(sep='', 'results/nowTime-1-0.0001-0.5-hd-',ped,'-ped-12-poi-6-pro-300-consumerdistance-+121000000000.0ns-duration.csv'))
...
for (ped in pedCount){
    f = file(paste(sep='', 'results/four-nowTime-1-0.0001-0.5-hd-',ped,'-ped-12-poi-6-pro-300-consumerdistance-+121000000000.0ns-duration.csv'))
    d = read.table(f,header=TRUE)
    d = subset(d, FaceDescr=="lte://" & (Type == "OutInterests" | Type == "OutData"))
    d[2:7] <- NULL
    d <- ddply(d, "Time", numcolwise(sum))
    d$Count <- ped
    d$InterestCount = "multiple"
    
    if(length(dataMul) == 0){
        dataMul =  d
    }
    else{
        dataMul = rbind(d,dataMul)
    }
    

}

data = rbind(data,dataMul)
...
for (ped in pedCount){
    for (r in 1:4){
        f = file(paste(sep='', 'results/four-nowTime-',r,'-0.0001-0.5-',den,'-',ped,'-ped-12-poi-6-pro-300-consumerdistance.csv'))
        d = read.table(f,header=TRUE)
        d = subset(d, FaceDescr=="lte://" & (Type == "OutInterests" | Type == "OutData"))
        d[2:7] <- NULL
        d <- ddply(d, "Time", numcolwise(sum))
...
dataMul$Type="Multiple Interest"
...
ggsave("graphs/pdfs/ld-comparison-basline-multi-single-traffic.pdf", plot=g, width=9, height=5, device=cairo_pdf)
...
data = c()
for (p in period){
    f = file(paste(sep='', 'results/nowTime-1-0.0001-0.5-',p,'-hd-40-ped-12-poi-6-pro-100-consumerdistance.csv'))
    d = read.table(f,header=TRUE)
...
detach(package:plyr)
ru <- data %>% group_by(Period) %>% summarise(PacketRaw = sum(PacketRaw))
...
detach(package:plyr)
du <- data %>% group_by(Period) %>% summarise(PacketRaw = sum(PacketRaw))

pp= c()
suppressPackageStartupMessages(library(plyr))
for (p in period){
    f= read.csv(paste(sep='','results/consumerCount-distance-100-',p,'.csv'))
