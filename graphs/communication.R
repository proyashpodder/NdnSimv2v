...
for (r in 1:nrow(ranges)) {
    for (run in 1:10) {
        tryCatch({
            ...
            f = file(paste(sep='', "results/3-rates-rates-run-", run ,"-min-", format(mi, nsmall=6),"-max-", format(ma, nsmall=6),".csv"))
...
data = subset(data, FaceDescr=="lte://" & (Type == "OutInterests" | Type == "InInterests" | Type == "OutData" | Type == "InData"))
