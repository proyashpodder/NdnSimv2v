...
int main (int argc, char *argv[])
{
  ...
  cmd.AddValue("consumer","", consumer);
 ...
 //Will add cost231Propagationloss model loss here for and packet loss
  ...
  for(int i=0;i<consumer;i++){
    consumerHelper.SetPrefix("/v2safety/8thStreet/0,0,0/700,0,0/100/src-"+std::to_string(i));
   consumerHelper.SetAttribute("Frequency", StringValue(std::to_string(frequency))); // 10 interests a second
   
     consumerHelper.Install(ueNodes.Get(i)).Start(Seconds(2));
   }
  ...
  Simulator::Stop(Seconds(2.99)); // expect 10 distinct requests
  int no = (int) consumer;
  std::ofstream of("results/nConsumer-packetLoss-"+std::to_string(no)+".csv");
