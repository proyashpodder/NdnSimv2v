...
int main (int argc, char *argv[])
{
  ...
  while(full < nodeNumber){
    ...
    if(check[no] == 0){
      ...
      xCoordinate = (no%160)*10;
      yCoordinate = (no/160)*4;
  ...
  for(int i= 0;i<nodeNumber; i++){
    ...
    if(i%2 == 0 ){
      ueNodes.Get (i)->GetObject<ConstantVelocityMobilityModel> ()->SetVelocity (Vector (2, 0, 0));
    }
    else if(i%3 == 0){
 ...
 //Will add cost231Propagationloss model loss here for and packet loss
  ...
  consumerHelper.SetPrefix("/v2safety/8thStreet/100,20,0/1550,0,0/100");
