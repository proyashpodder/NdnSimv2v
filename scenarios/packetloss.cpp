...
int main (int argc, char *argv[])
{
  ...
  cmd.AddValue("PscchTxPower","", PscchTxPower);
  ...
  Config::SetDefault ("ns3::LteUePowerControl::PscchTxPower", DoubleValue (PscchTxPower));
 // Config::SetDefault ("ns3::LteUePowerControl::PsschTxPower", DoubleValue (15.0));
  ...
  lteHelper->SetPathlossModelAttribute ("Frequency", DoubleValue(10));
    lteHelper->SetPathlossModelAttribute ("Lambda", DoubleValue(0.000001));
  //lteHelper->SetPathlossModelAttribute("MinDistance", DoubleValue(1000));
 ...
 //Will add cost231Propagationloss model loss here for and packet loss
  ...
  ::ns3::ndn::AppHelper consumerHelper("ns3::ndn::ConsumerCbr");
   ...
   consumerHelper.SetAttribute("Frequency", StringValue(std::to_string(frequency))); // 10 interests a second
   auto apps = consumerHelper.Install(ueNodes.Get(0));
    apps.Start(Seconds(2.00));
    apps.Stop(Seconds(2.99));
   // consumerHelper.Install(ueNodes.Get(1)).Start(Seconds(2));
  ...
  consumerHelper.SetAttribute("Batches", StringValue("2s 10")); // 10 interests a second
  ...
  Simulator::Stop(Seconds(9.99)); // expect 10 distinct requests
  /*int dis = (int) distance;
  int freq = (int) frequency;
  std::ofstream of("results/distance-frequency-packetLoss-"+std::to_string(dis)+"-"+std::to_string(freq)+".csv");
